...
const RecipePageListingRendering: React.FC<SimplePageListingProps> = ({ fields, rendering }) => {
  ...
  const filteredPages = useMemo(() => {
    const allPages = (rendering?.data || []) as RecipePageDataType[];
    ...
    const filteredResults = allPages.filter((page) => {
      ...
      const pageRecord = page as unknown as Record<string, RecipePageDataType>;
  ...
  return (
    <ContainedWrapper theme={effectiveTheme}>
      <div data-component="RecipePageListing" className="w-full py-10">
...
export default RecipePageListingRendering;
